...
export async function buildPersonalContext(args: BuildArgs): Promise<PersonalContext> {
  ...
  const [tasks, voiceNotes] = await Promise.all([
    // Trashed matters must never reach the prompt — the agent would happily
    // cite one back, and the user would rightly conclude the delete failed.
    Task.find({ userId: args.userId, ...notDeleted(), status: { $in: ['open', 'snoozed'] } })
