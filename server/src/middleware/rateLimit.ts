...
// Matters search compiles one Gemini round per submitted query (not per
// keystroke — the client only compiles on submit), so this sits alongside /ai/ask.
export const taskSearchLimiter = rateLimit({ windowMs: MINUTE_MS, max: 30 })
// Summaries scan a whole date range and are user-initiated one at a time.
export const taskSummaryLimiter = rateLimit({ windowMs: MINUTE_MS, max: 10 })
...
// Document scans are heavier still (multi-page vision calls) — capped lower
// than voice; the monthly quota (lib/documentScanQuota.ts) is the guard
// against sustained cost, this is only the burst guard.
export const documentScanLimiter = rateLimit({ windowMs: MINUTE_MS, max: 6 })
